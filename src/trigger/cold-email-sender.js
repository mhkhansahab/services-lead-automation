...
async function sentTodayCount() {
    ...
    const rows = await sbSelect(`outreach?select=id&status=in.(sent,delivered,opened,replied,bounced)&sent_at=gte.${encodeURIComponent(start)}&sent_at=lt.${encodeURIComponent(end)}`);
    return rows.length;
}
async function businessAlreadyContacted(businessId) {
...
export const smbColdEmailSender = schedules.task({
    ...
    run: async () => {
        ...
        const queue = await sbSelect(`v_ready_to_send?select=outreach_id,campaign_name,business_id,business_name,business_category,short_description,city,state,contact_name,email&limit=${remaining}`);
        const counters = {
            attempted: 0,
            sent: 0,
            skipped: 0,
