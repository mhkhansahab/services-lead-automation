...
function rotateQueries(queries, size, seed) {
    if (queries.length <= size)
        return queries;
    const out = [];
...
async function alreadySuppressed(email, domain) {
    if (email) {
        const byEmail = await sbSelect(`suppression_list?select=id&email=eq.${encodeURIComponent(email)}&limit=1`);
        if (byEmail.length > 0)
            return true;
    }
    if (domain) {
        const byDomain = await sbSelect(`suppression_list?select=id&domain=eq.${encodeURIComponent(domain)}&limit=1`);
        if (byDomain.length > 0)
            return true;
    }
    return false;
}
async function alreadyContacted(businessId) {
    const rows = await sbSelect(`outreach?select=id&business_id=eq.${businessId}&status=in.${CONTACTED_STATUSES}&limit=1`);
    ...
    run: async (payload) => {
        ...
        for (const query of selectedQueries) {
            ...
            for (const place of places) {
                ...
                const businessRows = await sbUpsert("businesses", {
                ...
                const contactRows = await sbUpsert("contacts", {
