...
function extractEmails(text: string, domain: string | null): string[] {
  ...
  if (!domain) return dedupe(normalized);
...
export async function enrichBusinessWebsite(website: string | null): Promise<{ domain: string | null; email: string | null }> {
  if (!website) return { domain: null, email: null };

  const domain = extractDomain(website);
  if (!domain) return { domain: null, email: null };
