...
function isMissingEventSyncRunsTable(error) {
    const message = String(error?.message ?? error);
    return message.includes("PGRST205") && message.includes("event_sync_runs");
}
async function latestSyncCutoff() {
    try {
        const rows = await sbSelect("event_sync_runs?select=run_started_at&order=run_started_at.desc&limit=1");
...
async function resolveRowsByRecipient(recipient, outreach, contactsById, businessesById) {
...
async function upsertSuppression(email, businessId, reason) {
    ...
    if (normalizedEmail) {
        const rows = await sbSelect(`suppression_list?select=${selectFields}&email=eq.${encodeURIComponent(normalizedEmail)}&limit=1`);
    ...
    if (domain) {
        const rows = await sbSelect(`suppression_list?select=${selectFields}&domain=eq.${encodeURIComponent(domain)}&limit=1`);
        const existing = rows[0];
        if (existing) {
            await sbUpdate(`suppression_list?id=eq.${existing.id}`, {
...
export async function runEventSync() {
    ...
    const [outreachRows, contacts, businesses] = await Promise.all([
        sbSelect("outreach?select=id,business_id,contact_id,status,provider_message_id"),
        sbSelect("contacts?select=id,email"),
        sbSelect("businesses?select=id,domain")
    ]);
    ...
    for (const emailEvent of allEmails) {
        ...
        if (cutoff && createdAt && createdAt <= new Date(cutoff))
            continue;
    ...
    try {
        ...
        warnings.push(`Google Sheets sync skipped: ${String(error.message ?? error)}`);
    }
    try {
